...
export default function TvShows() {
  const [shows, setShows] = React.useState<Movie[]>([]);
    ...
    const fetchShows = async () => {
      ...
      try {
        const data = selectedGenre 
          ? await discover('tv', selectedGenre, page)
          : await getPopular('tv', page);
    ...
    <div className="min-h-screen bg-zinc-950 pt-24 px-6 md:px-12 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-4xl font-black text-white tracking-tighter uppercase">TV Series</h1>
        </div>
        ...
        <div className="space-y-12">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {shows.map((movie, index) => (
              <div key={`${movie.id}-${index}`} ref={index === shows.length - 1 ? lastElementRef : null}>
